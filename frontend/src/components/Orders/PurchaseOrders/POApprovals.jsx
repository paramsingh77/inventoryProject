...
const POApprovals = () => {
  ...
  const fetchCompletePoDetails = async (poId) => {
    try {
      ...
      if (response.data) {
        ...
        console.log('🔍 POApprovals - Raw items from backend:', response.data.items);
        ...
        const items = (response.data.items || []).map(item => {
          ...
          return {
            ...
            name: item.name || item.device_model || 'Unnamed Item',
        ...
        // LOGGING: Show the productLink values after processing
        console.log('🔍 POApprovals - Processed items with productLink:', items.map(item => ({
          name: item.name,
  ...
  const convertToPoDocFormat = (po) => {
    ...
    const vendorPhone = po.phone_number || po.vendor_phone || po.phone || po.supplier_phone || 'Phone Not Available';
    ...
    // LOGGING: Show the productLink values in fallback processing
    console.log('🔍 convertToPoDocFormat - Fallback items with productLink:', items.map(item => ({
      name: item.name,
      productLink: item.productLink
    })));
