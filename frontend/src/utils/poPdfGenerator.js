...
// Generate PDF for Purchase Orders - Dedicated for PO List Viewing
export const generatePOPdfForViewing = async (poData, highQuality = false) => {
  ...
  try {
    ...
    console.log('Generating PDF canvas for PO viewing...');
    ...
    if (highQuality) {
      ...
      try {
        ...
        await testPromise;
        console.log('Generated valid PDF for viewing', pdfBlob.size, 'bytes');
        return pdfBlob;
        ...
        return generatePOPdfForViewing(poData, false);
      }
    } else {
...
export const generateOptimizedPOPdfForViewing = async (poData) => {
  // First try with high quality
  const highQualityPdf = await generatePOPdfForViewing(poData, true);
...
// Legacy function for backward compatibility - redirects to the new function
export const generatePOPdf = async (poData, highQuality = false) => {
  console.log('Using legacy generatePOPdf function, redirecting to generatePOPdfForViewing...');
  return generatePOPdfForViewing(poData, highQuality);
}; 